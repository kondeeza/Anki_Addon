...
Pinyin_SrcField = ''
HanziDeck_modelName = ''
HanziDeck_Input_HanziField = ''
HanziDeck_Output_SrcField = ''
OVERWRITE_DST_FIELD= ''
...
def reload_config():
    ...
    global HanziDeck_modelName
    global HanziDeck_Input_HanziField
    global HanziDeck_Output_SrcField
    ...
    modelName = config['03_01_modelName']
    Vocab_SrcField = config['03_02_Vocab_SrcField']
    Meaning_SrcField = config['03_03_Meaning_SrcField']
    Pinyin_SrcField = config['03_08_Pinyin_SrcField']
    HanziDeck_modelName = config['03_04_1_HanziDeck_modelName']
    HanziDeck_Input_HanziField = config['03_04_2_HanziDeck_Input_HanziField']
    HanziDeck_Output_SrcField = config['03_04_3_HanziDeck_Output_SrcField']
    OVERWRITE_DST_FIELD= config['03_05_OVERWRITE_DST_FIELD']
    Fuzzy_Character_Mode_Enabled= config['03_06_Fuzzy_Character_Mode_Enabled']
    debugMode = config['03_07_debugMode']
    query_input = config['03_09_query_input']
...
def BulkGenerateSimilarHanziVocabList(nids):
    ...
    warning_HanziDeck_Output_SrcField_NotFound = 0
    ...
    print ("Beginning with the following config:\n modelName: %s \n Vocab_SrcField: %s \n Meaning_SrcField: %s \n HanziDeck_Output_SrcField: %s \n OVERWRITE_DST_FIELD: %s \n Fuzzy_Character_Mode_Enabled: %s " %(str(modelName),Vocab_SrcField,Meaning_SrcField,HanziDeck_Output_SrcField,OVERWRITE_DST_FIELD,Fuzzy_Character_Mode_Enabled ))
    ...
    print ("--> Now on final part. Binding final output to     !")
    mw.progress.start()
    debugcount = 0
    ...
    for  loopno, nid in enumerate(nids):
        ...
        debugcount += 1
        note = mw.col.getNote(nid)
        if isinstance(HanziDeck_modelName, str):
            if HanziDeck_modelName not in note.model()['name']:
                if warning_ModelNotFound == 0:
                      print ("--> Model mismatch: %s vs %s" %( HanziDeck_modelName, note.model()['name']))
                warning_counter += 1
                warning_ModelNotFound += 1
                continue
        elif isinstance(HanziDeck_modelName, list):
            if not set(HanziDeck_modelName).isdisjoint(note.model()['name']):
                if warning_ModelNotFound == 0:
                      print ("--> Model mismatch: %s vs %s" %( str(HanziDeck_modelName), note.model()['name']))
                warning_counter += 1
                warning_ModelNotFound += 1
                continue
        src_Meaning = None
        src1 = None
        if HanziDeck_Input_HanziField  in note:
            src1 = HanziDeck_Input_HanziField 
        if not src1:
            # no src1 field
            continue
        dst = None
        if HanziDeck_Output_SrcField in note:
            dst = HanziDeck_Output_SrcField
        if not dst:
            continue
        if note[dst] and not OVERWRITE_DST_FIELD:
            # already contains data, skip
            #print ("--> %s not empty. Skipping!" % (HanziDeck_Input_HanziField ))
            continue
        try:
            ...
            for cur_Char in note[src1]:
                if cur_Char in HanziOfHanziFreqList:
                    ...
                    if not filteredLst:
                        #No vocab example found for this particular Hanzi
                        continue
    ...
    #showInfo ("--> Everything should have worked.\n warning_counter = %d \n warning_ModelNotFound = %d \n warning_Vocab_SrcField_NotFound = %d \n warning_Meaning_SrcField_NotFound = %d \n warning_HanziDeck_Output_SrcField_NotFound = %d" %(warning_counter,warning_ModelNotFound,warning_Vocab_SrcField_NotFound,warning_Meaning_SrcField_NotFound,warning_HanziDeck_Output_SrcField_NotFound))
    if (debugMode):
...
def setupMenu(browser):
    ...
    menu.addSeparator()
    a = menu.addAction('CN_03_Add_Example_Vocabs_To_Hanzi_Deck')
    a.triggered.connect(lambda _, b=browser: onBulkGenerateSimilarHanziVocabList(b,"manual"))
    q = menu.addAction('CN_03_Add_Example_Vocabs_To_Hanzi_Deck(QUERY)')
