...
if __package__:
    # called by script outside the package. added dot for importing relative module package.
    from .hangul_jamo import is_syllable, is_jamo_character, compose_jamo_characters, decompose_syllable, compose, decompose
else:
    # run as either __main__ for called by sibling files as __main__ (therefore __package__ is empty or None)
    from hangul_jamo import is_syllable, is_jamo_character, compose_jamo_characters, decompose_syllable, compose, decompose
...
def get_fuzzyJongseongList(inputStr):
    # Example , input is '가'
    # Output is  ['가']  # because no Jongseong
    # Example , input is '않'
    # Output is  ['안', '앉', '않']
...
def setupMenu(browser):
    ...
    menu.addSeparator()
    a = menu.addAction('KR_Find Hanguel Words With Similar Sound')
